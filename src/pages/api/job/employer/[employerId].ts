...
const getDashboardStats = async (employerId: string) => {
  ...
  const queries = {
    ...
    totalApplicants: `
      ...
      INNER JOIN jobs j ON aa.job_id = j.id
      WHERE j.employer_id = ?
    `,
    selectedApplicants: `
      SELECT COUNT(aa.id) AS count
      FROM applicant_applications aa
      INNER JOIN jobs j ON aa.job_id = j.id
      WHERE j.employer_id = ? AND aa.status = 'selected'
    `,
    totalViews: `SELECT SUM(views) AS count FROM jobs WHERE employer_id = ?`,
  ...
  try {
    ...
    throw new Error(`Database query failed: ${err.sqlMessage || err.message}`);
  }
};
