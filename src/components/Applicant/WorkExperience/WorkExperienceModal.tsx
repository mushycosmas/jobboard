...
interface Props {
  show: boolean;
  onClose: () => void;
  onSave: (data: Partial<WorkExperienceData>) => void;
...
const WorkExperienceModal: React.FC<Props> = ({
  ...
  const handleSaveClick = () => {
    ...
    if (
      ...
      alert(
        `Fill all required fields. Responsibilities must be at least ${MIN_CHAR_LIMIT} chars.`
      );
      return;
    }
  ...
  return (
    <Modal show={show} onHide={onClose} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{experience ? 'Edit Work Experience' : 'Add Work Experience'}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {/* Institution */}
        <Form.Group>
          ...
          <CreatableSelect
            ...
            onChange={(opt) =>
              setFormData((prev) => ({ ...prev, institution_id: opt?.value ?? opt?.label }))
            }
            value={
              formData.institution_id
                ? {
        ...
        <Form.Group className="mt-3">
          ...
          <CreatableSelect
            ...
            onChange={(opt) =>
              setFormData((prev) => ({ ...prev, position_id: opt?.value ?? opt?.label }))
            }
            value={
              formData.position_id
                ? {
