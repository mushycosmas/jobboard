...
<h1 style={{ fontSize: "28px" }}>{profile.fullName ?? "No Name"}</h1> <p className="text-muted mb-1">
{profile.email ?? "N/A"} | {profile.phone ?? "N/A"} </p> <p className="text-muted mb-1">{profile.address ?? "N/A"}</p> </div>
          ...
          {educationalQualifications.length ? (
            educationalQualifications.map((edu: any, idx: number) => (
              <ListGroup.Item key={idx}>
                <strong>{edu.degree ?? "N/A"}</strong> - {edu.institution ?? "N/A"} (
        ...
        <ListGroup variant="flush">
          {professionalQualifications.length ? (
            professionalQualifications.map((pq: any, idx: number) => (
              <ListGroup.Item key={idx}>
                {pq.course ?? pq.title ?? "N/A"} at {pq.institution ?? "N/A"} (
        ...
        <ListGroup variant="flush">
          {experiences.length ? (
            experiences.map((exp: any, idx: number) => (
              <ListGroup.Item key={idx}>
                <strong>{exp.position ?? exp.role ?? "N/A"}</strong> -{" "}
                {exp.institution ?? exp.company ?? "N/A"} (
                {exp.from ?? "N/A"} -{" "}
                {exp.to ?? (exp.is_currently_working ? "Present" : "N/A")})
              </ListGroup.Item>
            ))
          ) : (
        ...
        <Card.Body>
          {skills.length ? (
            skills.map((skill: any, idx: number) => (
              <Badge key={idx} bg="dark" className="me-1 mb-1">
